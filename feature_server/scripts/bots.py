...
def apply_script(protocol, connection, config):
    ...
    class BotConnection(connection):
        ...
        def join_game(self, team):
            consonants = ["J", "T", "M", "K", "W", "Z", "C", "N", "D", "L"]
            vowels = ["e", "a", "u"]
            titles = ["", "Ol'", "Yung"]
            finishes = ["rry", "ster"]
            self.name = '%(title)s %(consonant)s%(vowel)s%(finish)s' % { "finish": random.choice(finishes),"consonant": consonants[self.player_id % 10], "vowel": vowels[self.player_id % 3], "title": random.choice(titles) }
            self.fire_distance = 75
            self.terminator = True
            self.fire_tick_period = 15
            self.team = team

            weapons = [SMG_WEAPON, SMG_WEAPON, RIFLE_WEAPON]
            weapon = random.choice(weapons)

            if weapon == SMG_WEAPON:
                self.fire_distance = 75
            elif weapon == RIFLE_WEAPON:
                self.fire_distance = 90
            elif weapon == SHOTGUN_WEAPON:
                self.fire_distance = 45
            
            self.set_weapon( random.choice([SMG_WEAPON, RIFLE_WEAPON, SHOTGUN_WEAPON]), True)
        ...
        def think(self):
            ...
            if self.acquire_targets:
                for player in self.team.other.get_players():
                    if vector_collision(pos, player.world_object.position, 98.0):
                        if not self.aim_at:
            ...
            if location:
                ...
                self.input.add("jump")
                self.world_object.set_orientation(self.world_object.orientation.x, random.choice([1.0, -1.0]), 1.0)
                callLater(2, self.world_object.set_orientation, self.world_object.orientation.x, 0.0, 0.0)
        ...
        def update(self):
            ...
            if self.aim_at and self.aim_at.world_object:
                ...
                self.aim.set_vector(aim_at_pos + Vertex3(random.randrange(2), random.randrange(2), random.randrange(1)))
                self.aim -= pos
                distance_to_aim = self.aim.normalize() # don't move this line
            ...
            if self.aim_at and self.reaim_call == True:
                ...
                if not delta.is_zero():
                    ...
                    if (r or random.random() > 0.9) and self.ticks_fire > self.fire_tick_period and distance < self.fire_distance:
                        self.input.add("primary_fire")
                        self.input.discard("sprint")
                        valid_hit = self.world_object.validate_hit(self.aim_at.world_object,
                                TORSO, 5.0)
                        if not valid_hit:
                            return
                        position1 = self.world_object.position
                        position2 = self.aim_at.world_object.position
                        hit_amount = self.weapon_object.get_damage(
                            TORSO, position1, position2)
                        self.aim_at.hit(hit_amount, self, TORSO)
                        self.ticks_fire = 0
                    else:
                        self.ticks_fire += 1
